...
module.exports = (function() {
    //'use strict';
            ...
            app.post('/hrm/staff/gson',isAuthenticated,async(req,res) => { 
                    ...
                    let data = await Promise.all(rowsRec.map(async (row,i,array) =>{
                            ...
                            row.action = `<div class="btn-group" style="width:155px;margin-left:-50px;">
                                             ...
                                             ${req.session.user.adminrole == '08' ?`<a class="btn btn-default btn-sm" style="display:block;" title="Delete Staff Folder" href="/hrm/delstaff/${row.nox}" onclick="return confirm(\'Delete Staff?\');"><i class="fa fa-trash"></i></a>`:``}
                                          </div>`;            
                            }
                            return row;
                    }));
                ...
                var promo = {
                 ...
                 // Save Data 
                  if(req.body.id == 0){
                      ...
                      res.redirect('/hrm/staff/page/'+req.body.staff+'#page1'); 
                  }    
        ...
        app.get('/hrm/addstaff',isAuthenticated,isAdmin,async(req,res) => {           
            ...
            res.render('index_hr',{
                ...
                data: { bio: { id: 0,staff_group: group,exist}, doc: [{ id: 0 }], scales: rows[0], jobs: rows[1], units: rows[2], appoints: null},
                user: req.session.user               
            });            
        });
        ...
        app.get('/hrm/delstaff/:id',isAuthenticated,isAdmin,async(req, res) => {
            let id = req.params.id;
            console.log(id);
            // Log
            let sf = await dbx.query("select * from staff where staff_no = "+id);
            console.log(sf);
            let log = {staff_no:req.session.user.staff_no,action:'Staff Deleted : '+sf[0].staff_no,ipaddress:ip(req),datetime:new Date(),accesslevel:req.session.user.role,status:'1'};
            await dbx.query("insert into `log` set ?",log);
       ...
       app.get('/hrm/addcert/:staff',isAuthenticated,isAdmin,async(req,res) => { 
           ...
           // res.send("Welcome Back!");
            res.render('index_hr',{
                ...
                user: req.session.user,
                row: {id:0,staff_no: staff},specials,qualifies             
            });
        });
        ...
        app.get('/hrm/editcert/:id',isAuthenticated,isAdmin,async(req,res) => { 
         ...
         app.post('/hrm/postcert',isAuthenticated,isAdmin,cert,async(req,res)=>{       
            ...
            req.body.qualify_id == '' ? delete req.body.qualify_id : req.body.qualify_id; 
            req.body.specialize_id == '' ? delete req.body.specialize_id : req.body.specialize_id;
        ...
        app.get('/hrm/staff/page/:staff',isAuthenticated,isAdmin,async(req,res) => { 
            let id = req.params.staff;                           
            let rows = await dbx.query("select * from bank where staff_no ="+id+"; select *,ifnull(dob,'<b><em><small>NOT SET</small></em></b>') as dob,concat(fname,ifnull(concat(' ',mname),''),' ',lname) as name from staff_rel where staff_no=" + id + "; select c.*,q.title as program,z.title as specialize from certificate c left join qualification q on c.qualify_id = q.id left join specialization z on z.id = c.specialize_id where staff_no ="+id+"; select s.*,DATE_FORMAT(s.dob,'%Y-%m-%d') as dobx, DATE_FORMAT(s.appoint_date,'%Y-%m-%d') as first_appoint,DATE_FORMAT(p.appoint_date,'%Y-%m-%d') as appoint_date,DATE_FORMAT(s.confirm_date,'%Y-%m-%d') as first_confirm,concat(sc.grade,', STEP ',sc.notch) as grade from staff s left join promotion p on s.promo_id = p.id left join scale sc on p.scale_id = sc.id where s.staff_no = "+id);
            res.render('index_hr', {
                view: 'staffpage',
  ...
  /*  LEAVE MODULE ROUTING */
    ...
    app.get('/hrm/leave',isAuthenticated,isAdmin,async(req,res) => {              
       // let data = await dbx.query("select l.id as lid,year(applied_date) as year,date_format(hr_date,'%a %b %d, %Y') as hdate,date_format(head_date,'%a %b %d, %Y') as udate,date_format(start_date,'%a %b %d, %Y') as start,date_format(end_date,'%a %b %d, %Y') as end,date_format(resume_date,'%a %b %d, %Y') as resume,l.*,s.*,t.*,concat(r.lname,', ',r.fname,' ',r.mname,'<br>(',l.relieved_by,')') as relieve,concat(u.lname,', ',u.fname,' ',u.mname,'<br>(',l.head_endorse,')') as uhead,concat(h.lname,', ',h.fname,' ',h.mname,'<br>(',l.hr_endorse,')') as hhead from `leave_dump` l left join staff s on l.staff_no = s.staff_no left join leave_type t on t.id = l.type_id left join staff r on r.staff_no = l.relieved_by left join staff u on u.staff_no = l.head_endorse left join staff h on h.staff_no = l.hr_endorse");                  
        ...
        let holidays = await dbx.query("select *,date_format(holiday,'%a %d %b, %Y') as day from leave_exclude where (`year` = '"+new Date().getFullYear()+"' or `year` = '"+(new Date().getFullYear()+1)+"') order by `year` desc");
    ...
    app.post('/hrm/leavedump/gson',isAuthenticated,isAdmin,async(req,res) => { 
       ...
       // let sql = "select x.long_name,l.id as lid,year(applied_date) as year,date_format(hr_date,'%a %b %d, %Y') as hdate,date_format(head_date,'%a %b %d, %Y') as udate,date_format(start_date,'%a %b %d, %Y') as start,date_format(end_date,'%a %b %d, %Y') as end,date_format(resume_date,'%a %b %d, %Y') as resume,l.*,s.*,t.*,concat(r.lname,', ',r.fname,' ',r.mname,'<br>(',l.relieved_by,')') as relieve,concat(u.lname,', ',u.fname,' ',u.mname,'<br>(',l.head_endorse,')') as uhead,concat(h.lname,', ',h.fname,' ',h.mname,'<br>(',l.hr_endorse,')') as hhead from `leave_dump` l left join staff s on l.staff_no = s.staff_no left join leave_type t on t.id = l.type_id left join staff r on r.staff_no = l.relieved_by left join staff u on u.staff_no = l.head_endorse left join staff h on h.staff_no = l.hr_endorse left join promotion p on p.id = s.promo_id left join unit x on p.unit_id = x.id ";
        let sql = "select x.long_name,l.id as lid,year(applied_date) as year,date_format(hr_date,'%a %b %d, %Y') as hdate,date_format(head_date,'%a %b %d, %Y') as udate,date_format(start_date,'%a %b %d, %Y') as start,date_format(end_date,'%a %b %d, %Y') as end,date_format(resume_date,'%a %b %d, %Y') as resume,resume_date,l.id,relieved_by,approved_days,entitlement,applied_date,s.fname,s.mname,s.lname,s.staff_no,s.staff_no as nox,t.title,p.job_id,status,flag_resumed from `leave_dump` l left join staff s on l.staff_no = s.staff_no left join leave_type t on t.id = l.type_id left join promotion p on p.id = s.promo_id left join unit x on x.id = p.unit_id where l.status <> 'HEAD-PENDING' and l.status <> 'HR-PENDING' ";
    ...
    app.post('/hrm/leaveheadpend/gson',isAuthenticated,isAdmin,async(req,res) => { 
        console.log(req.body);         
        let sqlRec = "";
        let sqlTot = "";
        let where = "";           
    ...
    app.get('/hrm/editleave/:id',isAuthenticated,isAdmin,async(req,res) => {  
        ...
        var row = parseInt(id) > 0 ? await dbx.query("select l.*,date_format(start_date,'%Y-%m-%d') as start_date,p.job_id,p.staff_group from leave_dump l left join staff s on s.staff_no = l.staff_no left join promotion p on s.promo_id = p.id where l.id = "+id) : [{id:0}];  
        console.log(row);
        let types = await dbx.query("select * from leave_type where active = 1");  
        // Initialize Form
        res.render('index_hr',{
    ...
    app.get('/hrm/getleave/:staff/:type',isAuthenticated,async(req,res) => { 
        let staff = req.params.staff; 
        ...
        if(stud.length > 0){           
            var approved = await dbx.query("SELECT sum(approved_days) as ent FROM `leave_dump` where staff_no = "+staff+" and type_id = "+type+" and (status <> 'HR-REJECTED' and status <> 'HEAD-REJECTED') group by applied_date having date_format(applied_date,'%Y') ='"+(new Date().getFullYear())+"'");  
    ...
    app.post('/hrm/postleave',isAuthenticated,leave,loadmoment,async(req,res)=>{  
        ...
        let bio = {
            ...
            status : (req.session.user.page == 'staff'? 'HEAD-PENDING':'GRANTED')
        ...
        // Save Data   
        if(req.body.id == 0){
            // If Admin -- Head_endorse 000,Head_date now(),HR_endorse 000, hr_date now()
            if(req.session.user.page == 'admin'){
                bio.head_endorse = req.session.user.staff_no, bio.head_date = new Date(), bio.hr_endorse = req.session.user.staff_no, bio.hr_date = new Date();
            } console.log(bio);
            await dbx.query("insert into leave_dump set ?",bio);
        }else{ 
            req.session.user.page == 'staff' && req.body.urlto == 'st' ? bio.status = 'HEAD-PENDING' : delete bio.status;
        ...
        if(req.session.user.page == 'admin'){
            ...
            if(req.body.urlto == 'st'){ 
                // Applied by Staff
                res.redirect('/hrm/st/leave/hist');
            }else{ 
                // Applied by Head
        ...
        // View Leave Statistics for Staff
        app.get('/hrm/leave/chgreliever/:id',isAuthenticated,isAdmin,async(req,res) => { 
            var id = req.params.id; 
            var st = req.query.st; 
            ...
            if(oldst[0].relieved_by != null && parseInt(oldst[0].relieved_by) != 0){
                ...
                mailtheme('RELIEVER CHANGED','YOU HAVE BEEN RELIEVED OF LEAVE DUTIES',oldst[0].relieved_by,oldst[0].fname+' '+oldst[0].mname+' '+oldst[0].lname,oldst[0].photo,'UCC-HRMS || RELIEVER CHANGED','YOU HAVE BEEN RELIEVED OF LEAVE DUTIES',oldst[0].ucc_mail)
            }
            ...
            if(newst.length > 0 ){
                ...
                await dbx.query("insert into notification set ?",data);
                // Mail Staff
                mailtheme('LEAVE RELIEVER CHANGED','YOU HAVE BEEN PLACED ON RELIEVER DUTIES',newst[0].staff_no,newst[0].fname+' '+newst[0].mname+' '+newst[0].lname,newst[0].photo,'UCC-HRMS || RELIEVER CHANGED','YOUR HAVE BEEN PLACED ON LEAVE DUTIES AS A RELIEVER',newst[0].ucc_mail)
        ...
        // Test Moment business Days
         app.get('/hrm/testleave/',loadmoment,async(req,res) => { 
                var out = moment('05-12-2019','DD-MM-YYYY').businessAdd(1)._d;
       ...
       app.get('/hrm/leave/smshead/:id',async (req,res) => { 
            ...
            var msg = "Greetings, Please you have pending leave approvals in your staff portal dashboard. Goto http://hrms.ucc.edu.gh to approve them!";
            sms(mobile,msg);
       ...
       app.get('/hrm/leave/mailhead/:id',async (req,res) => { 
                ...
                var msg = "You have pending leave approvals of subordinates in your dashboard. Please do well to attend to these requests quickly! Goto https://hrms.ucc.edu.gh to access your staff portal.<br> Thank you very much.";
                var name = sl[0].name;
       ...
       app.get('/hrm/addlexclude/',isAuthenticated,isAdmin,async(req,res) => {            
        ...
        // Delete Holiday
        app.get('/hrm/dellexclude/:id',loadmoment,isAuthenticated,isAdmin,async(req, res) => {
             let id = req.params.id;
      ...
      // LEAVE BALANCES 
         ...
         // USERS & ROLES
            ...
            app.post('/hrm/postunit',isAuthenticated,isAdmin,async(req,res)=>{  
                ...
                //res.json({sub,old_sub,old_hod,hod});
                if((old_sub != null && sub != null) && old_sub != sub){
                     // Remove Sub Head access from old
                     let sl_old = await dbx.query("select s.staff_no,u.role,u.roles,u.id from staff s left join user u on s.staff_no = u.staff_no where (s.ucc_mail <> '' or s.ucc_mail IS NOT NULL) and s.staff_no = "+old_sub);  
                     if(sl_old.length > 0 && (sl_old[0].role != null && sl_old[0].roles != null)){
                        var ss = await dbx.query("select * from unit where unit_head = "+old_sub+" or unit_subhead = "+old_sub);
                        var roles = sl_old[0].roles.split(',').filter(row => {
                            return (row != '03' && row != '04');
                        });
                        if(ss.length == 1){
                            var new_roles = roles.length > 0 ? ('03,'+roles.join(',')) : '03';
                            await dbx.query("update user set roles = '"+new_roles+"',role = '03' where staff_no = "+old_sub); 
                        }else if(ss.length > 1){
                            var new_roles = roles.length > 0 ? ('04,'+roles.join(',')) : '04';
                            await dbx.query("update user set roles = '"+new_roles+"',role = '04' where staff_no = "+old_sub);
                        }
                    }

                     // Add new Sub Head Access to New Hod
                     let sl_sub = await dbx.query("select s.staff_no,u.role,u.roles,u.id from staff s left join user u on s.staff_no = u.staff_no where (s.ucc_mail <> '' or s.ucc_mail IS NOT NULL) and s.staff_no = "+sub);  
                     if(sl_sub.length > 0 && (sl_sub[0].role != null && sl_sub[0].roles != null)){
                        var roles = sl_sub[0].roles.split(',').filter(row => {
                            return (row != '03' && row != '04');
                        });
                        var new_roles = roles.length > 0 ? ('04,'+roles.join(',')) : '04';
                        await dbx.query("update user set roles = '"+new_roles+"',role = '04' where staff_no = "+sub); 
                     }
                }

                if(old_hod != null && hod != null && old_hod != hod){
                    // Remove Main Head access from old
                     let sl_hod = await dbx.query("select s.staff_no,u.role,u.roles,u.id from staff s left join user u on s.staff_no = u.staff_no where (s.ucc_mail <> '' or s.ucc_mail IS NOT NULL) and s.staff_no = "+old_hod);  
                     if(sl_hod.length > 0 && (sl_hod[0].role != null && sl_hod[0].roles != null)){
                        var ss = await dbx.query("select * from unit where unit_head = "+old_hod+" or unit_subhead = "+old_hod);
                        var roles = sl_hod[0].roles.split(',').filter(row => {
                            return (row != '03' && row != '04');
                        });
                        if(ss.length == 1){
                            var new_roles = roles.length > 0 ? ('03,'+roles.join(',')) : '03';
                            await dbx.query("update user set roles = '"+new_roles+"',role = '03' where staff_no = "+old_hod); 
                        }else if(ss.length > 1){
                            var new_roles = roles.length > 0 ? ('04,'+roles.join(',')) : '04';
                            await dbx.query("update user set roles = '"+new_roles+"',role = '04' where staff_no = "+old_hod);
                        }
                     }

                     // Add new Main Access to New Hod
                     let sl_sub = await dbx.query("select s.staff_no,u.role,u.roles,u.id from staff s left join user u on s.staff_no = u.staff_no where (s.ucc_mail <> '' or s.ucc_mail IS NOT NULL) and s.staff_no = "+hod);  
                     if(sl_sub.length > 0 && (sl_sub[0].role != null && sl_sub[0].roles != null)){
                        var roles = sl_sub[0].roles.split(',').filter(row => {
                            return (row != '03' && row != '04');
                        });
                        var new_roles = roles.length > 0 ? ('04,'+roles.join(',')) : '04';
                        await dbx.query("update user set roles = '"+new_roles+"',role = '04' where staff_no = "+hod); 
                     }
               }
            ...
            app.get('/hrm/appoint/data/:staffno',isAuthenticated,async(req,res) => {                
             ...
             app.get('/hrm/appoint/view/:staffno',isAuthenticated,isAdmin,async(req,res) => {                
                ...
                let data = await Promise.all(rows.map(async(row,i)=>{
                     // Renewal
                     row.renewal = await dbx.query("select * from renewal where staff_no = "+row.staff_no+" and promo_id = "+row.id+" order by id desc");
                     // Postretire
       ...
       app.get('/hrm/position/list/:post/:unit',isAuthenticated,async(req,res) => { 
        ...
        app.get('/hrm/transfer', async(req, res) => {
         ...
         app.get('/hrm/editransfer/:id',isAuthenticated,isAdmin,async(req,res) => { 
             let id = req.params.id;
             let rows = await dbx.query("select *,date_format(transfer_date,'%Y-%m-%d') as transfer_date from transfer where id = "+id);
             let units = await dbx.query("select * from unit where active = '1' order by trim(long_name)");
             console.log(rows);
             res.render('index_hr',{
                 view:"addtransfer",
         ...
         app.get('/hrm/transfer/list',isAuthenticated,async(req,res) => { 
                 ...
                 if(output == 'pdf'){
                     var sql = "select t.*,year(transfer_date) as year,concat(s.fname,ifnull(concat(' ',s.mname),''),' ',s.lname) as name,date_format(transfer_date,'%D %M, %Y') as transdate,s.photo,x.long_name as tounit,f.long_name as fromunit,j.title as jobtitle,t.id,s.staff_no as nox from transfer t left join staff s on s.staff_no = t.staff_no left join unit x on x.id = t.to_unit left join unit f on f.id = t.from_unit left join promotion p on s.promo_id = p.id left join job j on p.job_id = j.id where ";
                         sql += (year != '' ? " year(t.transfer_date) = "+year : '');
                         sql += (type != 'all' ? " and p.staff_group = '"+type+"' " : '');
                     ...
                     var sql = "select t.staff_no as 'Staff Number',concat(s.fname,ifnull(concat(' ',s.mname),''),' ',s.lname) as 'Full Name',date_format(transfer_date,'%D %M, %Y') as 'Transfer Date',x.long_name as 'Transferred To',f.long_name as 'Transferred From',j.title as 'Current Post' from transfer t left join staff s on s.staff_no = t.staff_no left join unit x on x.id = t.to_unit left join unit f on f.id = t.from_unit left join promotion p on s.promo_id = p.id left join job j on p.job_id = j.id where ";
                         sql += (year != '' ? " year(t.transfer_date) = "+year : '');
                         sql += (type != 'all' ? " and p.staff_group = '"+type+"' " : '');
                         sql += (month != 'all' ? " and month(t.transfer_date) = "+month : '');
                     var period = (type != 'all' ? (type == 'SM'?'SENIOR MEMBERS':(type=='SS'?'SENIOR STAFF':'JUNIOR STAFF'))+' - ' : "")+(year != '' ? year : '')+(month != 'all' ? "/"+dt.getMonth() : '');
 ...
 // NOTIFICATION MODULE  - STAFF && ADMIN
 app.get('/hrm/notes',isAuthenticated,async(req, res) => {
    const type = req.query.type;
    const data = req.query.data;
    const sys_no = await dbx.query("select * from notification where type = 'SYSTEM' and staff_no = "+req.session.user.staff_no);
    const box_no = await dbx.query("select * from notification where type = 'PERSONAL' and staff_no = "+req.session.user.staff_no);
    const cir_no = await dbx.query("select * from notification where type = 'CIRCULAR' and find_in_set("+req.session.user.staff_no+",members) > 0");
    let list = await dbx.query("select * from notification where staff_no = "+req.session.user.staff_no);
    let type_name,msg;
    if(type !== undefined){
       switch(type){
           case 'system': list = sys_no; type_name = "SYSTEM NOTIFICATIONS"; break;
           case 'inbox': list = box_no; type_name = "HRMS INBOX"; break;
           case 'circular': list = cir_no; type_name = "DHR CIRCULARS"; break;
           default : list = sys_no; type_name = "SYSTEM NOTIFICATIONS"; break;
        }
    }
    if(data !== undefined){
        let rec = await dbx.query("select * from notification where id = "+data);
        msg = rec[0];
     }else{
         msg = list[0];
     }
    ...
    res.render('partials/st_notes_new', {
        title: 'DISCIPLINARY RECORDS',
        no : [sys_no.length,cir_no.length,box_no.length],
        list,type_name,msg,
        user: req.session.user
    });
});
  ...
  /* STAFF MODULE ROUTING */
        ...
        // Head Default Page
         app.get('/hrm/st/head',isAuthenticated,async(req,res) => {  
            res.redirect('/hrm/st/head/member');
         });

         // Head Pages
         app.get('/hrm/st/head/:subnav',isAuthenticated,async(req,res) => { 
            let subnav = req.params.subnav;
            let data;
            switch(subnav){
                case "member": 
                      var rows = await dbx.query("select s.staff_no as nox,s.*,DATE_FORMAT(dob,'%d/%m/%Y') as dobx,b.*,u.long_name,sc.notch,sc.grade as title,j.title as jobtitle from staff s left join bank b on b.id = s.bank_id left join promotion p on p.id = s.promo_id left join scale sc on sc.id = p.scale_id left join unit u on p.unit_id = u.id left join job j on p.job_id = j.id where (u.unit_head = "+req.session.user.staff_no+" or u.unit_subhead = "+req.session.user.staff_no+")");
                      data = ['STAFF MEMBERS :  COUNT ( '+(rows.length)+' )','st_main_member',rows]; 
                      console.log(req.session.user); console.log(rows);
                      break;

                case "leave": 
                      var rows = await dbx.query("select l.id as lid,year(applied_date) as year,date_format(hr_date,'%a %b %d, %Y') as hdate,date_format(head_date,'%a %b %d, %Y') as udate,date_format(start_date,'%a %b %d, %Y') as start,date_format(end_date,'%a %b %d, %Y') as end,date_format(resume_date,'%a %b %d, %Y') as resume,l.*,s.*,t.*,concat(r.lname,', ',r.fname,' ',r.mname,'<br>(',l.relieved_by,')') as relieve,concat(u.lname,', ',u.fname,' ',u.mname,'<br>(',l.head_endorse,')') as uhead,concat(h.lname,', ',h.fname,' ',h.mname,'<br>(',l.hr_endorse,')') as hhead from `leave_dump` l left join staff s on l.staff_no = s.staff_no left join leave_type t on t.id = l.type_id left join staff r on r.staff_no = l.relieved_by left join staff u on u.staff_no = l.head_endorse left join staff h on h.staff_no = l.hr_endorse left join promotion p on p.id = s.promo_id left join unit x on p.unit_id = x.id where year(applied_date) = date_format(NOW(),'%Y') and (x.unit_head = "+req.session.user.staff_no+" or x.unit_subhead = "+req.session.user.staff_no+")");
                      data = ['STAFF LEAVE :  COUNT ( '+(rows.length)+' )','st_main_leave',rows]; 
                      break;

                case "memleave": 
                      var rows = await dbx.query("select s.staff_no as nox,s.*,DATE_FORMAT(s.dob,'%d/%m/%Y') as dobx,b.*,u.long_name,u.unit_head,u.unit_subhead,u.level,u.id as uid,sc.notch,sc.grade as title,j.title as jobtitle from staff s left join bank b on b.id = s.bank_id left join promotion p on p.id = s.promo_id left join scale sc on sc.id = p.scale_id left join unit u on ifnull(s.p_unit_id,p.unit_id) = u.id left join job j on p.job_id = j.id where (u.unit_head = "+req.session.user.staff_no+" or u.unit_subhead = "+req.session.user.staff_no+") and j.type = 'NON-ACADEMIC'");
                      var p2 = await dbx.query("select s.staff_no as nox,s.*,DATE_FORMAT(dob,'%d/%m/%Y') as dobx,u.long_name,u.unit_head,u.unit_subhead,j.title as jobtitle from staff s left join promotion p on p.id = s.promo_id left join unit u on ifnull(s.p_unit_id,p.unit_id) = u.id left join job j on p.job_id = j.id where (u.unit_head = "+req.session.user.staff_no+" or u.unit_subhead = "+req.session.user.staff_no+") and j.type = 'NON-ACADEMIC'");
                    
                      var lev = await dbx.query("select distinct(s.staff_no) as staff_no,l.id,status from `leave_dump` l left join staff s on l.staff_no = s.staff_no left join leave_type t on t.id = l.type_id left join staff r on r.staff_no = l.relieved_by left join staff u on u.staff_no = l.head_endorse left join staff h on h.staff_no = l.hr_endorse where (l.status = 'HEAD-PENDING' or l.status = 'GRANTED' or l.status = 'HR-PENDING') and year(applied_date) = date_format(NOW(),'%Y')");
                      rows = rows.map(row => {
                           row.onleave = false,row.status = '',row.lid = 0;
                           for(var st in lev){
                                if(lev[st].staff_no == row.nox){
                                    row.onleave = true,row.status = lev[st].status,row.lid = lev[st].id;
                                }
                           }    return row;
                      }); 
                      data = ['STAFF LEAVE :  COUNT ( '+(rows.length)+' )','st_main_memleave',rows]; 
                      break;
            }         
            res.render('index_st', {
                view: 'main',
         ...
         // Head - Place on Leave
         // Leave - Pages
         app.get('/hrm/st/head/placeleave/:staff',isAuthenticated,async(req,res) => { 
                var staff = parseInt(req.params.staff); 
                var urlto = 'ad';
                var row =  [{id:0,staff_no:staff}];  
                console.log(urlto);
                console.log(row);
                let types = await dbx.query("select * from leave_type where active = 1");  
                data = ['LEAVE APPLICATION','st_leave_form',0,types,row[0],urlto]; 
                res.render('index_st', {
                    view: 'leave',
                    title: 'STAFF LEAVE',
                    user: req.session.user,
        ...
        // Head - Approve Leave - Unit Head       
        app.get('/hrm/st/head/approve/:id',isAuthenticated,async(req,res)=>{
            var id = req.params.id;
            var done = await dbx.query("update leave_dump set status = 'HR-PENDING',head_endorse = "+req.session.user.staff_no+",head_date = NOW() where id ="+id);
            
             // Notification
             let st = await dbx.query("select s.*,l.* from leave_dump l left join staff s on l.staff_no = s.staff_no where l.id ="+id);
             let data = {action : 'Leave Approval - Unit Head',message : 'Leave application with STAT(D:'+st[0].approved_days+',E:'+st[0].entitlement+') pending approval by head is approved at '+st[0].head_date, staff_no : st[0].staff_no, datetime : new Date(),read_flag : '0',priority : '1'}
             await dbx.query("insert into notification set ?",data);
        ...
        // Head - Decline Leave - Unit Head       
        app.get('/hrm/st/head/decline/:id',isAuthenticated,async(req,res)=>{
            var id = req.params.id;
            var done = await dbx.query("update leave_dump set status = 'HEAD-REJECTED', head_endorse = "+req.session.user.staff_no+", head_date = NOW() where id ="+id);
           
             // Notification
             let st = await dbx.query("select s.*,l.* from leave_dump l left join staff s on l.staff_no = s.staff_no where l.id ="+id);
             let data = {action : 'Leave Declined - Unit Head',message : 'Leave application with STAT(D:'+st[0].approved_days+',E:'+st[0].entitlement+') pending approval by head is declined at '+st[0].head_date, staff_no : st[0].staff_no, datetime : new Date(),read_flag : '0',priority : '1'}
             await dbx.query("insert into notification set ?",data);
        ...
        // Head - Upgrade Leave - Unit Head       
        app.get('/hrm/st/head/upgrade/:id',isAuthenticated,async(req,res)=>{
            var id = req.params.id;
            var note = decodeURI(req.query.note); 
            await dbx.query("update leave_dump set status = 'STAFF-PENDING', head_remark = '"+note+"' where id ="+id);

             // Notification
             let st = await dbx.query("select s.*,l.* from leave_dump l left join staff s on l.staff_no = s.staff_no where l.id ="+id);
             let data = {action : 'Leave Correction - Unit Head ('+new Date()+')',message : 'Leave application with STAT(D:'+st[0].approved_days+',E:'+st[0].entitlement+') pending approval by head needs correction or update by applicant, Please read the Head\'s note below.\n\n ( Head Note : '+st[0].head_remark+' )', staff_no : st[0].staff_no, datetime : new Date(),read_flag : '0',priority : '1'}
             await dbx.query("insert into notification set ?",data);
        ...
        /*//  LEAVE  - STAFF MODULE  //*/
         ...
         // Staff Leave - Default Page
         app.get('/hrm/st/leave',isAuthenticated,async(req,res) => {  
              res.redirect('/hrm/st/leave/hist');
              // Disclaimer off 
              req.session.user.disclaim = false;   
         });
         ...
         // Leave - Pages
         app.get('/hrm/st/leave/:subnav',isAuthenticated,async(req,res) => { 
            let subnav = req.params.subnav;
            let data;
            switch(subnav){
                case "hist": 
                      var rows = await dbx.query("select l.id as lid,year(applied_date) as year,date_format(hr_date,'%a %b %d, %Y') as hdate,date_format(head_date,'%a %b %d, %Y') as udate,date_format(start_date,'%a %b %d, %Y') as start,date_format(end_date,'%a %b %d, %Y') as end,date_format(resume_date,'%a %b %d, %Y') as resume,l.*,s.*,t.*,concat(r.lname,', ',r.fname,' ',r.mname,'<br>(',l.relieved_by,')') as relieve,concat(u.lname,', ',u.fname,' ',u.mname,'<br>(',l.head_endorse,')') as uhead,concat(h.lname,', ',h.fname,' ',h.mname,'<br>(',l.hr_endorse,')') as hhead from `leave_dump` l left join staff s on l.staff_no = s.staff_no left join leave_type t on t.id = l.type_id left join staff r on r.staff_no = l.relieved_by left join staff u on u.staff_no = l.head_endorse left join staff h on h.staff_no = l.hr_endorse left join promotion p on p.id = s.promo_id where year(applied_date) = date_format(NOW(),'%Y') and s.staff_no = "+req.session.user.staff_no);
                      var all = await dbx.query("select l.id as lid,year(applied_date) as year,date_format(hr_date,'%a %b %d, %Y') as hdate,date_format(head_date,'%a %b %d, %Y') as udate,date_format(start_date,'%a %b %d, %Y') as start,date_format(end_date,'%a %b %d, %Y') as end,date_format(resume_date,'%a %b %d, %Y') as resume,l.*,s.*,t.*,concat(r.lname,', ',r.fname,' ',r.mname,'<br>(',l.relieved_by,')') as relieve,concat(u.lname,', ',u.fname,' ',u.mname,'<br>(',l.head_endorse,')') as uhead,concat(h.lname,', ',h.fname,' ',h.mname,'<br>(',l.hr_endorse,')') as hhead from `leave_dump` l left join staff s on l.staff_no = s.staff_no left join leave_type t on t.id = l.type_id left join staff r on r.staff_no = l.relieved_by left join staff u on u.staff_no = l.head_endorse left join staff h on h.staff_no = l.hr_endorse left join promotion p on p.id = s.promo_id where s.staff_no = "+req.session.user.staff_no);
                      data = ['LEAVE HISTORY','st_leave_hist',rows,all]; 
                      break;

                case "stat": 
                      var rows = await dbx.query("select l.* from leave_dump l where l.staff_no = "+req.session.user.staff_no);
                      data = ['LEAVE STATISTICS','st_leave_stat',rows]; 
                      break;

                case "view": 
                      var id = req.query.id;
                      data = ['LEAVE APPLICATION FORM','st_leave_view',id]; 
                      break;

                case "form": 
                      var id = parseInt(req.query.id) || 0; 
                      var urlto = req.query.urlto || 'ad';
                      var row = parseInt(id) > 0 ? await dbx.query("select *,date_format(start_date,'%Y-%m-%d') as start_date from leave_dump where id = "+id) : [{id:0}];  
                      console.log(urlto);
                      console.log(row);
                      let types = await dbx.query("select * from leave_type where active = 1");  
                      data = ['LEAVE APPLICATION','st_leave_form',id,types,row[0],urlto]; 
                      break;
            }      
            // Get Leave Data
            var url = req.protocol+'://'+(process.env.appType == 'local' && process.env.PORT == 8080 ? 'localhost:'+process.env.PORT : 'hrms.ucc.edu.gh')+'/hrm/getleavedata/'+req.session.user.staff_no+'/2';
            request({method:'get',json:true,url}, function (err, rex, body) {   
               res.render('index_st', {
                    view: 'leave',
                    title: 'STAFF LEAVE',
                    user: req.session.user,
        ...
        app.get('/hrm/st/getltypes',isAuthenticated,async(req,res) => {  
            var id = parseInt(req.query.id) || 0; 
            var st = parseInt(req.query.staff) || 0; 
            console.log(id);
            if(st == 0){
                var row = parseInt(id) > 0 ? await dbx.query("select l.id as lid,p.*,l.*,date_format(start_date,'%Y-%m-%d') as start_date,s.gender,concat(s.fname,' ',ifnull('',s.mname),' ',s.lname) as name from leave_dump l left join staff s on s.staff_no = l.staff_no left join promotion p on p.id = s.promo_id where l.id = "+id) : [{id:0,staff_no:req.session.user.staff_no,staff_group:req.session.user.staff_group,gender:req.session.user.gender,staff_status:req.session.user.staff_status,name:(req.session.user.fname+" "+req.session.user.mname+" "+req.session.user.lname),urlto:'st'}];  
                var relieves = await dbx.query("select s.staff_no,concat(s.fname,' ',ifnull('',s.mname),' ',s.lname) as name from staff s join promotion p on p.id = s.promo_id left join unit u on p.unit_id = u.id where u.unit_head = "+req.session.user.staff_no);
            }else{
                var ss = await dbx.query("select p.*,s.gender,s.staff_status,s.staff_no,concat(s.fname,' ',ifnull('',s.mname),' ',s.lname) as name from staff s left join promotion p on p.id = s.promo_id where s.staff_no = "+st)
                var row = parseInt(id) > 0 ? await dbx.query("select l.id as lid,p.*,l.*,date_format(start_date,'%Y-%m-%d') as start_date,s.gender from leave_dump l left join staff s on s.staff_no = l.staff_no left join promotion p on p.id = s.promo_id where l.id = "+id) : [{id:0,staff_no:st,staff_group:ss[0].staff_group,gender:ss[0].gender,staff_status:ss[0].staff_status,name:ss[0].name,urlto:'st'}];  
                var relieves = await dbx.query("select s.staff_no,concat(s.fname,' ',ifnull('',s.mname),' ',s.lname) as name from staff s join promotion p on p.id = s.promo_id left join unit u on p.unit_id = u.id where u.unit_head = "+req.session.user.staff_no);
            }
            console.log(row);

            var types; 
            if(row[0].staff_status != 'TEMPORAL'){
                if(row[0].gender == 'F'){
                    types = await dbx.query("select * from leave_type where active = 1");   
                }else{
                    types = await dbx.query("select * from leave_type where title <> 'MATERNITY' and active = 1");   
                }
            }else{
                types = await dbx.query("select * from leave_type where title = 'CASUAL' and active = 1"); 
            }

            //types = await dbx.query("select * from leave_type where title <> 'MATERNITY' and active = 1");   
            res.json({types,row,relieves});
         });


         // Leave - Delete Form
         app.get('/hrm/st/leavedel',isAuthenticated,async(req,res) => {  
            var id = req.query.id || 0;
            await dbx.query("delete from leave_dump where id = "+id)
            res.redirect('/hrm/st/leave/hist');
             // Disclaimer off 
             req.session.user.disclaim = false;   
         });
         ...
         // Staff - Get Biodata
        app.get('/hrm/st/biodata/:id',isAuthenticated,async(req,res) => {  
            var id = req.params.id; 
            var row = await dbx.query("select s.*,p.*,s.id from staff s left join promotion p on p.id = s.promo_id where s.staff_no = "+id);  
            console.log(row);
            res.json({row});
        });
        ...
        // Post Biodata Form
        app.post('/hrm/postbiodata',isAuthenticated,async(req,res)=>{  
            console.log(req.body);        
            let ins = await dbx.query("update staff set ? where staff_no ="+req.body.staff_no,req.body);  
           // console.log(ins);                    
            // Redirect
            res.redirect('/hrm/dash');
            //res.json(req.body);  
         ...
         /*//  TRANSFER  - STAFF MODULE  //*/

         // Staff Transfer - Default Page
         app.get('/hrm/st/transfer',isAuthenticated,async(req,res) => {  
            res.redirect('/hrm/st/transfer/request');
            // Disclaimer off 
            req.session.user.disclaim = false;   
         });
        ...
        // Transfer - Pages
        app.get('/hrm/st/transfer/:subnav',isAuthenticated,async(req,res) => { 
            let subnav = req.params.subnav;
            let data;
            switch(subnav){
                case "request": 
                        var rows = await dbx.query("select t.*,concat(s.fname,ifnull(concat(' ',s.mname),''),' ',s.lname) as name,date_format(t.applied_date,'%D %M, %Y') as appliedate,s.photo,x.long_name as tounit,f.long_name as fromunit,j.title as jobtitle,t.id,s.staff_no as nox from transfer_request t left join staff s on s.staff_no = t.staff_no left join unit x on x.id = t.to_unit left join promotion p on s.promo_id = p.id left join unit f on f.id = p.unit_id left join job j on p.job_id = j.id  where t.staff_no = "+req.session.user.staff_no);
                        data = ['TRANSFER REQUESTS','st_transfer_request',rows]; 
                        break;

                case "stat": 
                        var rows = await dbx.query("select t.* from transfer t where t.staff_no = "+req.session.user.staff_no);
                        data = ['TRANSFER STATISTICS','st_transfer_stat',rows]; 
                        break;
            }         
            res.render('index_st', {
                view: 'transfer',
        ...
        /*  NOTIFICATION  - STAFF MODULE  */

         // Staff Notifications - Default Page
         app.get('/hrm/st/notes',isAuthenticated,async(req,res) => {  
            res.redirect('/hrm/st/notes/list');
         });

        // Notification - Pages
        app.get('/hrm/st/notes/:subnav',isAuthenticated,async(req,res) => { 
          let subnav = req.params.subnav;
          let data;
          switch(subnav){
              case "list": 
                    var rows = await dbx.query("select *,date_format(datetime,'%a, %M %d %Y %h:%i') as logtime from notification where staff_no = "+req.session.user.staff_no+" order by id desc");
                    data = ['NOTIFICATION','st_notes_list',rows]; 
                    break;

              case "full": 
                    var id = parseInt(req.query.id) || 0; 
                    var rows = await dbx.query("select *,date_format(datetime,'%a, %M %d %Y %h:%i') as logtime  from notification where id = "+id);
                    await dbx.query("update notification set read_flag ='1' where id = "+id);
                    data = ['MESSAGE','st_notes_full',rows[0]]; 
                    break;

              case "page": 
                    var next = req.query.page;
                    var rows = await dbx.query("select *,date_format(datetime,'%a, %M %d %Y %h:%i') as logtime  from notification where staff_no = "+req.session.user.staff_no+" order by id desc limit "+page);
                    data = ['NOTIFICATION','st_notes_list',rows]; 
                    break;
        ...
        app.get('/hrm/st/chgpass/:pass',isAuthenticated,async(req,res) => {  
            var pass = md5(req.params.pass.trim()); 
            await dbx.query("update user set password = ? where staff_no = "+req.session.user.staff_no,[pass]);  
            req.session.destroy();   
            res.redirect('/hrm/dash');
        });


        /* SENIOR MEMBERS PAPERS */
        app.get('/hrm/st/paper',isAuthenticated,async(req,res) => { 
            let rows = await dbx.query("select * from paper where staff_no = "+req.session.user.staff_no);
            res.render('index_st', {
                view: 'paper',
                title: 'ALL PAPERS',
                user: req.session.user,
         ...
         /* PROMOTIONS - STAFF */
          ...
          app.get('/hrm/st/promotion',isAuthenticated,async(req,res) => {  
            res.redirect('/hrm/st/promotion/stat');
            // Disclaimer off 
            req.session.user.disclaim = false;   
         });

         app.get('/hrm/st/promotion/:subnav',isAuthenticated,async(req,res) => { 
            let subnav = req.params.subnav;
            let data;
            switch(subnav){
                case "request": 
                        var rows = await dbx.query("select * from promotion_request where staff_no = "+req.session.user.staff_no);
                        data = ['PROMOTION REQUESTS','st_promotion_request',rows]; 
                        break;

                case "stat": 
                        var rows = await dbx.query("select p.* from promotion p where p.staff_no = "+req.session.user.staff_no);
                        data = ['PROMOTION STATISTICS','st_promotion_stat',rows]; 
                        break;
            }         
            res.render('index_st', {
                view: 'promotion',
        ...
        /* ASSETS MANAGEMENT INTEGRATION */
        app.get('/hrm/st/mainreq',isAuthenticated,async(req,res) => { 
            var rows = await dbx.query("select t.*,concat(s.fname,ifnull(concat(' ',s.mname),''),' ',s.lname) as name,date_format(t.applied_date,'%D %M, %Y') as appliedate,s.photo,x.long_name as tounit,f.long_name as fromunit,j.title as jobtitle,t.id,s.staff_no as nox from transfer_request t left join staff s on s.staff_no = t.staff_no left join unit x on x.id = t.to_unit left join promotion p on s.promo_id = p.id left join unit f on f.id = p.unit_id left join job j on p.job_id = j.id  where t.staff_no = "+req.session.user.staff_no);
            res.render('index_st', {
                view: 'mainreq',
         ...
         app.get('/hrm/st/docman',isAuthenticated,async(req,res) => { 
            var rows = await dbx.query("select t.*,concat(s.fname,ifnull(concat(' ',s.mname),''),' ',s.lname) as name,date_format(t.applied_date,'%D %M, %Y') as appliedate,s.photo,x.long_name as tounit,f.long_name as fromunit,j.title as jobtitle,t.id,s.staff_no as nox from transfer_request t left join staff s on s.staff_no = t.staff_no left join unit x on x.id = t.to_unit left join promotion p on s.promo_id = p.id left join unit f on f.id = p.unit_id left join job j on p.job_id = j.id  where t.staff_no = "+req.session.user.staff_no);
            res.render('index_st', {
                view: 'docman',
    ...
    // Transfer Request Record - Get Leave Types
    app.get('/hrm/st/getransfereq',isAuthenticated,async(req,res) => {  
        var id = parseInt(req.query.id) || 0; 
        var row = await dbx.query("select *,date_format(start_date,'%Y-%m-%d') as start_date from transfer_request where id = "+id); 
        if(row.length > 0){
        ...
        app.post('/hrm/postnss',isAuthenticated,isAdmin,nssphoto,async(req,res)=>{ 
           ...
           */
           ...
           if(req.body.id <= 0){  
               ...
               const msg = `Please goto the https://hrms.ucc.edu.gh/nss/portal, Login with Token : ${req.body.token} and your NSS Number`;
               sms(mobile,msg);
           }else{  
       ...
       app.get('/hrm/nss/file/:id',isAuthenticated,async(req,res) => { 
        ...
        // Fetch Halls & ID
        app.get('/hrm/hallapi',isAuthenticated,async(req,res) => {  
            //var ms = require('../testhall.json');
            // Get All Senior members
            let sm = await dbx.query("select count(*) as no from staff s left join promotion p on s.promo_id = p.id where p.staff_group = 'SM'");
            var url = 'http://192.168.0.22/hallapi';
            var options = {
                method: 'get',
                json: true,
                url: url
            }
            request(options, async function (err, resx, body) {
                var keys = Object.keys(body);
                let values = await Promise.all(keys.map(async (row,i) => {
                    let data = body[row];
                    const ratio =  data.quota; 
                    const hallno =  Math.round((ratio/100) * sm[0].no);
                    let ux = await dbx.query("select count(*) as no from ext_hall where hall_id = '"+data.hallid+"'");
                    //if(data.hallid == 'SRC') ux[0].no = 112;
                    data.visible = (ux[0].no < hallno) ? true : false;
                    return data;
                }));
                //data = values;
                console.log(values);
        ...
        // Post Hall Data
        app.post('/hrm/hallapi',isAuthenticated,async(req,res) => {  
            let st = await dbx.query("select * from ext_hall where staff_no ="+req.session.user.staff_no);
            if(st.length <= 0){
                const data = {staff_no:req.session.user.staff_no,hall_id:req.body.hall_id};
                console.log(data);
                const sl = await dbx.query("insert into ext_hall set ?", data); 
                //var sl = true;
                if(sl){
                    res.json({success:true,msg:'Record saved successfully!'});
                }else{
                    res.json({success:true, msg: 'Record update failed!'});
                }
            }else{
                res.json({success:false, msg:'Staff already has a hall affliation!'});
            }
         });
         

         // Get Hall Affiliates
         app.get('/hrm/hallmembers',isAuthenticated,async(req,res) => {  
            let st = await dbx.query("select s.staff_no as 'STAFF NO',concat(s.fname,' ',ifnull(s.mname,''),' ',s.lname) as 'FULL NAME',s.gender as 'GENDER',u.long_name as 'UNIT',j.title as 'DESIGNATION',s.ucc_mail as 'EMAIL',e.hall_id as 'HALL ID' from ext_hall e left join staff s on s.staff_no = e.staff_no left join promotion p on p.id = s.promo_id join unit u on u.id = p.unit_id left join job j on j.id = p.job_id");
            res.json(st);
         });  

         app.get('/hrm/hallmembersexcel',isAuthenticated,async(req,res) => {  
            let st = await dbx.query("select s.staff_no as 'STAFF NO',concat(s.fname,' ',ifnull(s.mname,''),' ',s.lname) as 'FULL NAME',s.gender as 'GENDER',u.long_name as 'UNIT',j.title as 'DESIGNATION',s.ucc_mail as 'EMAIL',e.hall_id as 'HALL ID' from ext_hall e left join staff s on s.staff_no = e.staff_no left join promotion p on p.id = s.promo_id left join unit u on u.id = p.unit_id left join job j on j.id = p.job_id");
            res.xls('halldata.xlsx',st);
         });

    
        /* API CALLS  FOR THIRD PARTY APPS */
        
        // Get Hall Affiliates
        app.get('/hrm/staffapi/:sno',async(req,res) => {  
            let sno = req.params.sno;
            let st = await dbx.query("select s.staff_no as 'STAFF NO',concat(s.fname,' ',ifnull(s.mname,''),' ',s.lname) as 'FULL NAME',s.gender as 'GENDER',u.long_name as 'UNIT',j.title as 'DESIGNATION',s.ucc_mail as 'EMAIL' from staff s left join promotion p on p.id = s.promo_id left join unit u on u.id = p.unit_id left join job j on j.id = p.job_id where s.staff_no = "+sno);
            res.json(st);
        });  
        ...
        // Password Change -- Staff
         app.get('/hrm/st/livechat',isAuthenticated,async(req,res) => {  
